...
function SegmentedTimerGrid({ timeLeft, colorTheme }: { timeLeft: TimeLeft; colorTheme: 'violet' | 'indigo' | 'emerald' | 'rose' }) {
  const themeClasses = {
    violet: {
      bg: 'bg-[#6C4CF5]',
      text: 'text-[#6C4CF5]',
  ...
  return (
    <div className="grid grid-cols-4 gap-2 text-center font-mono">
      <div className={`bg-white border ${themeClasses.border} p-2 rounded-2xl shadow-sm ${themeClasses.glow} transition-all duration-300`}>
        ...
        <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block mt-1">Days</span>
      </div>
      <div className={`bg-white border ${themeClasses.border} p-2 rounded-2xl shadow-sm ${themeClasses.glow} transition-all duration-300`}>
        <span className={`block font-black text-lg sm:text-xl ${themeClasses.num} leading-none tabular-nums`}>
          {String(timeLeft.hours).padStart(2, '0')}
        </span>
        <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block mt-1">Hours</span>
      </div>
      <div className={`bg-white border ${themeClasses.border} p-2 rounded-2xl shadow-sm ${themeClasses.glow} transition-all duration-300`}>
        <span className={`block font-black text-lg sm:text-xl ${themeClasses.num} leading-none tabular-nums`}>
          {String(timeLeft.minutes).padStart(2, '0')}
        </span>
        <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block mt-1">Mins</span>
      </div>
      <div className={`bg-white border ${themeClasses.border} p-2 rounded-2xl shadow-sm ${themeClasses.glow} transition-all duration-300`}>
        <span className={`block font-black text-lg sm:text-xl ${themeClasses.text} leading-none tabular-nums animate-pulse`}>
          {String(timeLeft.seconds).padStart(2, '0')}
        </span>
        <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block mt-1">Secs</span>
      </div>
    </div>
  );
}
    ...
    >
      ...
      <div className="flex justify-between items-start gap-2 mb-4.5">
        <div className="flex flex-wrap gap-1.5">
          {card.isPinned && (
            <span className="text-[10px] bg-[#6C4CF5]/10 text-[#6C4CF5] font-extrabold px-2.5 py-0.5 rounded-full border border-[#6C4CF5]/20 flex items-center gap-1 shadow-sm">
              📌 PINNED
            </span>
          )}
      ...
      <div className="flex-1 mb-5">
        ...
        <div className="grid grid-cols-2 gap-2.5 mb-4.5 font-sans text-xs">
          <div className="bg-white/90 border border-indigo-100/60 p-2.5 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] flex items-center gap-2">
            ...
            <div>
              <span className="block text-[9px] text-slate-400 font-black uppercase tracking-wide">Exam Date</span>
              <span className="font-bold text-slate-800">
                {isExamValid ? examTarget.toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) : 'To Be Declared'}
              </span>
            </div>
          </div>
          <div className="bg-white/90 border border-violet-100/60 p-2.5 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] flex items-center gap-2">
            ...
            <div>
              <span className="block text-[9px] text-slate-400 font-black uppercase tracking-wide">Last Apply</span>
              <span className="font-bold text-slate-800">
                {isApplyValid ? applyTarget.toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) : 'To Be Declared'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="relative flex items-center justify-between mt-4 mb-5.5 px-2">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <span className="text-[9px] font-black mt-1.5 text-slate-500 uppercase tracking-tight">Circular</span>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <span className="text-[9px] font-black mt-1.5 text-slate-500 uppercase tracking-tight">Apply Closes</span>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <span className="text-[9px] font-black mt-1.5 text-slate-500 uppercase tracking-tight">Exam Day</span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {currentStage === 'apply_open' && isApplyValid && (
            <>
              ...
              <SegmentedTimerGrid timeLeft={applyTime} colorTheme="violet" />
              <div className="flex items-center gap-1.5 pt-1.5 text-[10px] text-slate-400 justify-center font-bold font-mono">
                <span>• Exam Scheduled: {examTarget?.toLocaleDateString('en-US', {month: 'short', day: 'numeric'})} ({examTime.days}d {examTime.hours}h left)</span>
              </div>
            </>
          )}
          ...
          {currentStage === 'exam_prep' && isExamValid && (
            <>
              ...
              <SegmentedTimerGrid timeLeft={examTime} colorTheme="indigo" />
              <div className="flex items-center gap-1 pt-1.5 text-[10px] text-indigo-600 justify-center font-extrabold">
                <AlertCircle className="w-3 h-3" />
                <span>Registration has closed. Preparation phase active.</span>
              </div>
            </>
          )}
          ...
          {currentStage === 'exam_ended' && (
            <div className="bg-emerald-50/60 border border-emerald-100 text-emerald-800 p-4 rounded-2xl text-center shadow-inner space-y-1.5">
              ...
              <p className="text-[10px] text-slate-500 font-medium">
                The official exam was held on {examTarget?.toLocaleDateString('en-US', {month: 'long', day: 'numeric', year: 'numeric'})}. Live answers, cut-off calculators, and results can be accessed below.
              </p>
            </div>
          )}
          ...
          {!isExamValid && !isApplyValid && (
            <div className="bg-slate-50 border border-slate-200 text-slate-600 p-4 rounded-2xl text-center space-y-1">
              <div className="font-extrabold text-xs">TBD (Dates to be announced)</div>
              <p className="text-[10px] text-slate-400 font-medium">
                The circular for this exam session has not been released yet. Pin this page or subscribe to newsletters to receive immediate updates.
              </p>
            </div>
          )}
        </div>
