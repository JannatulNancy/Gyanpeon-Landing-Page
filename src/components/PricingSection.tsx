      ...
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-[700px] mx-auto mb-16">
          ...
          <p className="mt-4 text-slate-500 text-base leading-relaxed">
            Unlock complete study materials, unlimited MCQ practice, dynamic AI teachers, and precise analytical mock test centers.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
          {visiblePlans.map((plan, index) => (
            <motion.div
              key={plan.id}
              className={`group bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-xl border transition-all duration-300 flex flex-col justify-between relative ${
                plan.isActive 
            ...
            >
                  ...
                  {plan.image ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-tr from-purple-100 to-indigo-100 flex items-center justify-center relative">
                      ...
                      <div className="absolute bottom-4 text-center">
                        <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">{plan.name}</span>
                      </div>
                    </div>
                  )}
              ...
              <div className="p-6 pt-0">
                <button
                  onClick={() => alert(`Redirecting to purchase portal for ${plan.name}...`)}
                ...
                >
                  {plan.isActive ? 'Access Active Course' : plan.buttonText}
                </button>
              </div>
        ...
        <div className="mt-12 bg-white rounded-2xl p-4 border border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 max-w-[900px] mx-auto shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs text-slate-500 font-semibold leading-relaxed">
              Have you already purchased a batch on our platform? Make sure you are logged in to access the study planners and unlimited smart exams instantly.
            </p>
          </div>
